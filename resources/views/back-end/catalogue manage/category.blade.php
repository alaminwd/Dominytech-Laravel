                            ...
                            <table class="table table-hover category_table">
                                ...
                                <tbody>
                                    ...
                                    <tr class="text-center">
                                        ...
                                        <td>{{$category->slug}}</td>
                                        <td>{{$category->short_desp}}</td>
                                        {{-- <td><a href="#" class="btn btn-{{$category->status == 0 ? 'light':'success'}}">{{$category->status == 0 ? 'inactive':'active'}}</a></td> --}}
                                        <td> <img width="100" height="60" style="border-radius: 50%" src="{{asset('upload/category_images')}}/{{$category->image}}" alt=""></td>
                                        <td>
                                            
                                            <a href="{{route('category.edit', [$category->slug, $category->id ])}}" class="btn btn-info edit_category">Edit
                                                <!-- Edit Icon SVG -->
                                            ...
                                            <button data-id="{{$category->id}}" class="btn btn-danger delete_category"><svg xmlns="" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg></button>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
...
@extends('back-end.catalogue manage.add_category')
...
<script>
        ...
        if (!category || !short_description || !slug || !image) {
    ...
    $(document).on('click', '.delete_category', function(e){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('delete.category') }}", // Make sure this route expects a POST/DELETE request
